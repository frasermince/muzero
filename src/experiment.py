...
def get_experiment_class(memory_actor, params_actor, sample_actor):
    ...
    class MuzeroExperiment(experiment.AbstractExperiment):
        ...
        def _muzero_loss_fn(
            ...
            value, policy, reward, hidden_state, new_key = network.forward_observation(
                self.key, params, actions[:, 0, :].astype(float), observations)
            values = jnp.zeros_like(value)
            values = jnp.expand_dims(values, axis=0)
            values = jnp.resize(values, (self.rollout_size + 1,
            ...
            # with torch.no_grad():
            scalar_values = support_to_scalar(
                jnp.stack(values).transpose(1, 0, 2))
        ...
        def _eval_epoch(self, rng):
            ...
            for inputs in self._build_eval_input():
                ...
                if summed_scalars is None:
                    ...
                    summed_scalars = jax.tree_multimap(
                        jnp.add, summed_scalars, scalars)

            mean_scalars = jax.tree_map(
    ...
    def rollout_function(i, data):
        ...
        value, policy, reward, hidden_state, _ = network.forward_hidden_state(
            second_key, params, actions[:, i, :].astype(float), hidden_state)
        return_values = return_values.at[i].set(value)
        # hidden_state.register_hook(lambda grad: grad * 0.5)
        current_value_loss, current_policy_loss, current_reward_loss = losses(
            i, value, policy, reward, support_value, search_policy, support_reward)
    ...
    def losses(i, value, policy, reward, support_value, search_policy, support_reward):
        current_policy_loss = optax.softmax_cross_entropy(
            policy, search_policy.transpose(1, 0, 2)[i, :, :])
        current_value_loss = optax.softmax_cross_entropy(
            value.squeeze(), support_value[:, i, :])
        current_reward_loss = optax.softmax_cross_entropy(
            reward, support_reward[:, i, :])
        return current_value_loss, current_policy_loss, current_reward_loss

    return MuzeroExperiment
