...
def _log_outputs(step, scalar_values):
    ...
    def f_list(x): return x.tolist() if isinstance(x, jnp.ndarray) else x
    logging.info("global_step: %d, %s", step,
                 jax.tree_map(f_list, scalar_values))
    ...
    if is_chief:
        if writer is not None:
            def write_scalars(global_step: int, scalar_values):
                writer.write_scalars.remote(global_step, scalar_values)
...
def evaluate(
    ...
    if config.one_off_evaluate:
        ...
        if writer is not None:
            writer.write_scalars.remote(state.global_step, scalar_values)
